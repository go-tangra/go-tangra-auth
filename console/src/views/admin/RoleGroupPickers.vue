...
<template>
  <fieldset :data-test="`${prefix}-roles`" :disabled="disabled">
    ...
    <UiCheckbox v-for="r in roles" :id="`${prefix}-role-${r.id}`" :key="r.id ?? ''" :model-value="roleIds.includes(r.id ?? '')" :label="r.display_name ?? r.slug ?? ''" @update:model-value="roleIds = toggle(roleIds, r.id ?? '', $event)" />
  </fieldset>
