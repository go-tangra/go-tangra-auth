...
describe('groups console', () => {
  ...
  it('lists groups, validates the create dialog and confirms deletion with the member count', async () => {
    ...
    const w = mount(Groups, { global: { plugins: plugins() }, attachTo: document.body })
    await flushPromises()
    ...
    await flushPromises()
    const dialog = document.body.querySelector('[data-test="group-dialog"]')!
    ;(dialog.querySelector('[data-test="save-group"]') as HTMLElement).click()
    await flushPromises()
    expect(fetch).not.toHaveBeenCalledWith('/api/v1/admin/groups', expect.objectContaining({ method: 'POST' }))
    const input = dialog.querySelector('[data-test="group-name-input"] input') as HTMLInputElement
    input.value = ' Ops '
    ...
    await flushPromises()
    const del = document.body.querySelector('[data-test="delete-dialog"]')!
    expect(del.querySelector('[data-test="delete-summary"]')!.textContent).toContain('2 members')
    ;(del.querySelector('[data-test="confirm-delete"]') as HTMLElement).click()
    await flushPromises()
  ...
  it('shows members and roles of a group, adds and removes a member, saves roles', async () => {
    ...
    const w = mount(GroupDetail, { global: { plugins: plugins() }, attachTo: document.body })
    await flushPromises()
